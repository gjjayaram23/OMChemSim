within Simulator.UnitOperations.DistillationColumn;
  ...
  model DistTray "Model of a tray used in distillation column"
    ...
    Real T(unit = "K", min = 0, start = T_N[1]) "Temperature";
    ...
    Real Fout(unit = "mol/s", min = 0) "Sidedraw molar flow";
    Real Fvap_s[2](each unit = "mol/s", each min = 0,start={V_O,V1[1]}) "Vapor molar flow";
    Real Fliq_s[2](each unit = "mol/s", each min = 0,start={L1[2],L1[1]}) "Liquid molar flow";  
    Real xout_c[Nc](each unit = "-", each min = 0, each max = 1) "Components mole fraction at sidedraw";
    Real xvap_sc[2, Nc](each unit = "-", each min = 0, each max = 1, start={Xb, x[1, :]}) "Components vapor mole fraction";
    Real xliq_sc[2, Nc](each unit = "-", each min = 0, each max = 1, start={x[2, :], x[1, :]}) "Components liquid mole fraction";
    ...
    Real x_pc[3, Nc](each min =0, each max = 0);
    
    Real Pdew(unit = "Pa", min = 0) "Dew pressure";
    Real Pbubl(unit = "Pa", min = 0) "Bubble pressure";
