within Simulator.UnitOperations.DistillationColumn;
  ...
  model Cond "Model of a condenser used in distillation column"
    ...
    Real P(unit = "K", min = 0, P_N) "Pressure";
    Real T(unit = "Pa", min = 0, Tdt) "Temperature";
    Real Fin(unit = "mol/s", min = 0) "Feed molar flow rate";
    Real xin_c[Nc](each unit = "-", each min = 0, each max = 1) "Feed components mole fraction"; 
    Real xvapin_c[Nc](each unit = "-", each min = 0, each max = 1) "Inlet components vapor molar fraction"; 
    Real Hin(unit = "kJ/kmol") "Feed inlet molar enthalpy";
   
    Real Fout(unit = "mol/s", min = 0, start = D1) "Side draw molar flow";
    Real Fvapin(unit = "mol/s", min = 0, start = V1[2]) "Inlet vapor molar flow";
    Real Fliqout(unit = "mol/s", min = 0, start = L_O) "Outlet liquid molar flow";
    Real xout_c[Nc](each unit = "-", each min = 0, each max = 1, start = Xd) "Side draw components mole fraction";
    Real xliqout_c[Nc](each unit = "-", each min = 0, each max = 1, start = Xd) "Outlet components liquid mole fraction";
